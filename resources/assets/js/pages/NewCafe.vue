    ...
    <form>
        <div class="grid-container">
           ...
           <div class="grid-x grid-padding-x" v-for="(location, key) in locations" :key=key>
                ...
                <div class="large-12 medium-12 small-12 cell">
                    <label>支持的冲泡方法</label>
                    <span class="brew-method" v-for="brewMethod in brewMethods" :key="brewMethod.id+'-'+key">
...
import {mapState} from 'vuex';
  export default {
    ...
    computed:{
        ...mapState({
            brewMethods:state=>state.brewMethods.brewMethods,
            cafeAddStatus:state=>state.cafes.cafeAddStatus
        })
    },
    watch:{
        'cafeAddStatus':function(){
            if(this.cafeAddStatus === 2){
                // 添加成功
                this.clearForm();
                $("#cafe-added-successfully").show().delay(5000).fadeOut();
            }
            if(this.cafeAddStatus === 3){
                 // 添加失败
